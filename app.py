...
with st.sidebar:
    ...
    if "LDA" in mode:
        ...
        st.info(
            "**🎯 关键词定向分析**\n\n"
            "适合已知产品关键词。选择主题（如音质、续航、操作）统计每个主题的提及次数与权重。"
        )
    st.divider()
...
if "LDA" in mode:
    ...
    if st.session_state['lda_results']:
        ...
        st.caption(
            "💡 点击上方按钮切换主题，右侧条形图实时更新。悬停可查看精确权重值。"
        )
        st.info(
            "**📖 如何判断模型质量**\n\n"
            "**① 气泡分布**：气泡之间距离越远，说明主题区分度越好；大面积重叠则说明主题相似，建议减少主题数。\n\n"
            "**② 气泡大小**：各主题占比相对均匀为佳（最大/最小不超过3~5倍）；若一个主题占70%以上，说明主题数设置过多或数据不够丰富。\n\n"
            "**③ 关键词权重**：右侧条形图中排名靠前的词应该具有业务意义（如「音质」「降噪」），且不同主题之间的高权重词重叠应尽量少。"
        )
        ...
        if csv_file is None:
            ...
            with st.status("⏳ 正在分析，请稍候…", expanded=True) as status:
                try:
                    ...
                    result_df, stats_df, bar_fig, pie_fig, example_dict, summary = run_analysis(
