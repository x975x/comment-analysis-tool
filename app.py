...
if "LDA" in mode:
        ...
        if csv_file is None:
            ...
            with st.status("⏳ 正在分析，请稍候…", expanded=True) as status:
                try:
                    ...
                    # 4. 构建语料库
                    st.write("📦 构建词典与 TF-IDF 语料库…")
                    dictionary, bow_corpus, tfidf_corpus = build_corpus(docs, no_below, no_above)
                    if len(dictionary) == 0:
                        status.update(label="❌ 词典为空", state="error")
                        st.error("过滤后词典为空，请降低「词频下限」或提高「词频上限」。")
                        st.stop()
                    st.write(f"✅ 词典大小：**{len(dictionary)}** 个词")
                    ...
                    if auto_k:
                        ...
                        best_k, scores, chart_bytes, lda_model = compute_coherence_scores(
                            tfidf_corpus, dictionary, docs, krange[0], krange[1]
                        )
                        final_k = best_k
                        st.write(f"✅ 最佳主题数：**{best_k}**")
                    else:
                        st.write(f"🧠 训练 LDA 模型（{num_topics} 个主题，passes={passes}）…")
                        lda_model = train_lda_model(tfidf_corpus, dictionary, num_topics, passes)
                        final_k = num_topics
                        st.write("✅ 模型训练完成")

                    # 6. 生成可视化
                    st.write("🎨 生成 pyLDAvis 交互可视化…")
                    topics_df = get_topics_df(lda_model, num_words)
                    html_str  = get_lda_vis_html(lda_model, bow_corpus, dictionary)
    ...
    if st.session_state['lda_results']:
        ...
        if res['chart_bytes']:
            st.markdown("### 📈 Coherence Score 曲线")
        ...
        # pyLDAvis 嵌入
        st.markdown("### 🎨 LDA 交互式可视化")
        st.caption(
            "💡 点击左侧气泡查看对应主题的关键词分布；"
            "拖动右侧「相关性」滑块可调整关键词排序权重。"
        )
        components.html(res['html_str'], height=830, scrolling=True)
        st.download_button(
            "⬇️ 下载 LDA 可视化（HTML 文件）",
            data=res['html_str'].encode('utf-8'),
